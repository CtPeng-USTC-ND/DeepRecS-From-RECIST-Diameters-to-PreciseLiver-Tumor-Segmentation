...
def orthometric_mean_squared_error(y_true, y_pred):
    ...
    for i in range(batch_size):
        b_c, b_r, l_c, l_r, r_c, r_r, t_c, t_r = get_point(i, y_pred)
        pred_pts = tf.convert_to_tensor([b_c, b_r, l_c, l_r, r_c, r_r, t_c, t_r], 'float32')
        b_c_, b_r_, l_c_, l_r_, r_c_, r_r_, t_c_, t_r_ = get_point(i, y_true)
        gt_pts = tf.convert_to_tensor([b_c_, b_r_, l_c_, l_r_, r_c_, r_r_, t_c_, t_r_], 'float32')
...
def get_point(i, y_pred):
    ...
    return b_c, b_r, l_c, l_r, r_c, r_r, t_c, t_r
