...
def CGBS_Net(input_shape=(256, 256, 4), classes=2, backbone='xception', OS=16, alpha=1., rate=3):
    ...
    x2 = BilinearUpsampling(output_size=(int(np.ceil(h * rate)),
                                         int(np.ceil(w * rate))))(x2)
    ...
    skip2 = BilinearUpsampling(output_size=(int(np.ceil(h * rate)),
                                            int(np.ceil(w * rate))))(skip2)
